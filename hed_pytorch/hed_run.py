...
def estimate(model,tensorInput):
	...
	return model(tensorInput.cuda().view(1, 3, intHeight, intWidth))[0, :, :, :].cpu()
# end
...
if __name__ == '__main__':
	...
	moduleNetwork = HED_Network()
	moduleNetwork.load_state_dict(torch.load('./network-' + arguments_strModel + '.pytorch'))
	...
	tensorOutput = estimate(moduleNetwork,tensorInput)
